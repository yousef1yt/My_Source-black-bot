...
async def reply_with_link(client, message):
    ...
    await message.reply_text("‹ تم تحديد الهمسة اضغط لارسال الهمسة ›", reply_markup=reply_markup)
...
async def display_hms(client, callback):
    ...
    if hmses.get(str(who_id)) is not None:
        ...
        await callback.answer("عيل خبيث داخل تشوف الهمسه ليه؟", show_alert=True)
